...
def manager(monkeypatch):
    ...
    # timestamp2date is resolved by check_linbo_backup_date_or_404, in
    # utils.checks, not by this router module.
    monkeypatch.setattr(checks, "timestamp2date", lambda ts: BACKUP_DATE)
    return instance
...
def test_delete_backup_400s_on_a_malformed_timestamp(manager, monkeypatch):
    ...
    monkeypatch.setattr(checks, "timestamp2date", reject)
    manager.groups = {"win11": make_group(backups={BACKUP_DATE: Mock()})}
...
def test_rename_400s_on_an_invalid_target_name(manager):
    manager.groups = {"win11": make_group()}
